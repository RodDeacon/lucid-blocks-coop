...
func _physics_process(delta: float) -> void :
    ...
    if not is_instance_valid(target) or dead or disabled or not is_session_position_loaded(global_position):
        return
    ...
    if state == CHASE:
        ...
        if Ref.sun.target_time_scale >= 1.0:
            ...
            %IdleTimer.start(randf_range(idle_time_min, idle_time_max))
            return
        var center: Vector3 = target.head.global_position if is_instance_valid(target.head) else target.global_position
        ...
        SpatialMath.look_at( %RotationPivot, center)
...
func teleport(final: bool) -> bool:
    ...
    var target_head: Vector3 = target.head.global_position if is_instance_valid(target.head) else target_position
    ...
    if spawn_position.distance_to(target_position) <= near_radius:
        return false
    ...
    if final:
        SpatialMath.look_at( %RotationPivot, target_head)
